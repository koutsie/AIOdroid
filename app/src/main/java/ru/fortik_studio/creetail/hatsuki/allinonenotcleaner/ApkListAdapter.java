...
public class ApkListAdapter extends RecyclerView.Adapter<ApkListAdapter.ViewHolder> {
  ...
  public ApkExtractorActivity mActivity;
  public final PackageManager packageManager;

  int names_to_load = 0;

  private Map<String, String> cache_appName = Collections
      .synchronizedMap(new LinkedHashMap<String, String>(10, 1.5f, true));
  private Map<String, Drawable> cache_appIcon = Collections
      .synchronizedMap(new LinkedHashMap<String, Drawable>(10, 1.5f, true));

  private String search_pattern;

  public ApkListAdapter(ApkExtractorActivity activity) {
  ...
  class AppNameLoader implements Runnable {
    ...
    @Override
    public void run() {
      cache_appName.put(package_info.packageName,
  ...
  static class ViewHolder extends RecyclerView.ViewHolder implements OnClickListener {
    ...
    private void setAndHighlight(TextView view, String value, String pattern) {
      ...
      for (int offset = 0, index = value.indexOf(pattern, offset);
          index >= 0 && offset < value.length(); index = value.indexOf(pattern, offset)) {
        Spannable span = new SpannableString(view.getText());
        span.setSpan(new ForegroundColorSpan(Color.BLUE), index, index + pattern.length(),
  ...
  public void onBindViewHolder(ViewHolder holder, int i) {
    ...
    if (cache_appIcon.containsKey(item.packageName) && cache_appName
  ...
  private void filterListByPattern() {
    ...
    for (PackageInfo info : list_original) {
      ...
      do {
        ...
        if (cache_appName.containsKey(info.packageName) && cache_appName.get(info.packageName)
            .toLowerCase().contains(search_pattern)) {
